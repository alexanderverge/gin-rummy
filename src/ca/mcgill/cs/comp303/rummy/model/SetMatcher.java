...
public class SetMatcher implements Serializable
{
	 ...
	 */
	public Set<ICardSet> matchSets(List<Card> pCards)
	{
		aCards = pCards;
	...
	/**
	 * Set the cards of the SetMatcher, for testing.
	 * @param pCards The cards.
	 */
	...
	/**
	 * Construct all possible combinations of valid sets.
	 * @return The Players sets.
	 */
	...
	/**
	 * Construct all possible combination of valid runs.
	 * @return The Player's runs.
	 */
	public Set<ICardSet> getAllRuns()
	{
		...
		for (int i = 0; i <= aCards.size() - 3; i++)
		{
			...
			allRuns.addAll(SetMatcher.getRunCombinations(potentialRun));
		}
	...
	/**
	 * Construct all possible combinations of valid groups.
		...
		for (int i = 0; i <= aCards.size() - 3; i++)
		{
			...
			if (possibleSet.isGroup())
			{
				...
				if (i + 3 < aCards.size())
				{
					...
					{
						allGroups.addAll(SetMatcher.getGroupCombinations(aCards.subList(i, i + 4)));
						i += 3;
						continue;
					}
				}
	...
	/**
	 * Construct group combinations.
	 * @param group A group of cards
	 * @return All possible sub-groupings.
	 */
	private static Collection<ICardSet> getGroupCombinations(List<Card> group)
	{
	...
	/**
	 * Construct run combinations.
	 * @param potentialRun Cards that may constitute a run.
	 * @return All possible run subsets.
	 */
	private static Collection<ICardSet> getRunCombinations(List<Card> potentialRun)
	{
		Collection<ICardSet> combinations = new HashSet<ICardSet>();
